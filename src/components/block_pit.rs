    ...
    Effect::new(move |_| {
        ...
        *g.borrow_mut() = Some(Box::new(move || {
            ...
            for i in 0..block_count {
                ...
                if objs[i].x > container_width - objs[i].width {
                    objs[i].x = container_width - objs[i].width;
                    objs[i].vx *= -0.5;
                }
            }
            ...
            for i in 0..block_count {
                for j in (i + 1)..block_count {
                    ...
                    if distance < min_dist {
                        ...
                        objs[i].vx += push_x * 0.2;
                        objs[i].vy += push_y * 0.2;
                        objs[j].vx -= push_x * 0.2;
                        objs[j].vy -= push_y * 0.2;
                    }
                }
            }
                ...
                if let Some(el) = document().get_element_by_id(&format!("phys-block-{}", i)) {
                    if let Ok(el) = el.dyn_into::<HtmlElement>() {
                        let _ = el.style().set_property(
                            "transform",
                            &format!(
                                "translate({:.1}px, {:.1}px) rotate({:.1}deg)",
        ...
        let start_f = g.clone();
        request_animation_frame(move || {
            if let Some(callback) = start_f.borrow().as_ref() {
                callback();
            }
        });
    });
